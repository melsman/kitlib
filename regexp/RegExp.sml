...
fun add m i v =
    let val v = case Map.lookup m i of
                    NONE => v
                  | SOME v' => Set.union(v',v)
    in Map.add(i,v,m)
    end
...
fun dft n ass fol Epsilon = 
  ...
  | dft n ass fol (Atom a) = 
    (false, Set.singleton n, Set.singleton n, n+1, Map.add(n,a,ass), fol)
...
fun dftp n ass fol par Epsilon = 
  ...
  | dftp n ass fol par (Atom a) = 
    (false, Set.singleton n, Set.singleton n, n+1, Map.add(n,a,ass), fol, par, (nil,nil))
...
fun nfa n ass fol start : char atom NFA =
  {start = start, 
   edges = Vector.tabulate (n,  mapLookup ass),
   trans = Vector.tabulate (n-1, mapLookup fol),
   final = n-1}
	   ...
	   let val new = trans sub x
	       val P = Set.fold (fn (n,P) =>
				 let val r = parnum rightpar (x,n)
				   ...
				   val p = mapLookup P0 x handle NotFound => raise Fail "one"
				   val p = paradd a p
				   val p = parpop r p
				   val p = parpush l p
				 in Map.add(n,p,P)
...
local
  ...
  fun nfarun_par (nfa as {nfa={start,final,...},...} : char atom NFAP, (Pl,Pr)) string =
    ...
    in  if Set.member(endstates,final) then SOME (rev(map(implode o rev) (#1(mapLookup P final))))
	else NONE
    end 
